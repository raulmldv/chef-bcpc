...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def plug_vifs(self, instance, network_info):
        ...
        for vif in network_info:
            # tstachecki: If the nova-compute is restarting, we need to retain
            # the existing MTU already set for the VIF. This is provided via
            # the instance configuration. If not for this, we derive MTU from
            # the Neutron network, which may mismatch with how the instance is
            ...
            try:
                ...
                iface = guest.get_interface_by_cfg(cfg)
                network = vif.get('network')
                if network:
                    new_network = copy.deepcopy(network)
                    new_network._set_meta({'mtu': iface.mtu})
                    vif['network'] = new_network
