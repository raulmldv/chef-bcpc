...
def _watch(resp, callback):
    for decoded_line in resp.iter_content(chunk_size=None,
                                          decode_unicode=True):
        # decode_unicode will decode according to the Content-Encoding header
        # if one is present.  In the case it's not, we still require a string:
        # so assume UTF-8 in such cases.
        if not isinstance(decoded_line, six.string_types):
