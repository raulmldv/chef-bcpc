    ...
    def _trash_purge(self) -> None:
        ...
        with RADOSClient(self) as client:
            for vol in self.RBDProxy().trash_list(client.ioctx):
                try:
                    # NOTE(jinggeng): clone info can only be retrieved from a rbd.Image object,
                    # and generating a rbd.Image object from a trashed volume will cause an error
                    # of "RBD image not found". In order to retrieve the clone information,
                    # a volume needs to be first restored from trash, and then deleted.
                    # Although RBDProxy().remove is slower than RBDProxy().trash_remove, the
                    # restored volume is deleted asynchronously so it's still an improvement.
                    vol_name = utils.convert_str(vol.get('name'))
                    self.RBDProxy().trash_restore(client.ioctx,
                                                  vol.get('id'),
                                                  vol_name)
                    _, parent, parent_snap = self._get_clone_info(self.rbd.Image(client.ioctx,
                                                                                 vol_name),
                                                                  vol_name)
                    ...
                    LOG.info("Deleted %s from trash for backend '%s'",
                             ...
                             self._backend_name)
                    if parent:
                        LOG.info("Volume '%s' is a clone so cleaning references", vol.get('name'))
    ...
    def delete_volume(self, volume: Volume) -> None:
        ...
        with RADOSClient(self) as client:
            ...
            if clone_snap is None:
                ...
                # references. If deferred deletion is enabled, delete references
                # when the volume is purged.
                if parent and not self.configuration.enable_deferred_deletion:
